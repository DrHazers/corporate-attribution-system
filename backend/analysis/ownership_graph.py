...
def _load_current_relationship_rows(
    ...
    *,
    ...
    return list(
        db.execute(
            text(CURRENT_DIRECT_RELATIONSHIPS_SQL),
            {
                "as_of_date": date.today().isoformat(),
                "direct_only": 1 if direct_only else 0,
...
def _serialize_relationship(
    ...
    holding_ratio = _normalize_decimal(relationship["holding_ratio"])
    ...
    has_numeric_ratio = infer_has_numeric_ratio(
        ...
        holding_ratio=holding_ratio,
        has_numeric_ratio=bool(relationship["has_numeric_ratio"]),
    )
    ...
    return {
        ...
        "effective_date": _normalize_date_text(relationship["effective_date"]),
        "expiry_date": _normalize_date_text(relationship["expiry_date"]),
...
def _load_current_relationship_map(
    ...
    for relationship in _load_current_relationship_rows(db, direct_only=True):
